...
class ElmoLstm(_EncoderBase):
    ...
    def __init__(self, input_size, hidden_size, cell_size, num_layers, requires_grad=False,
                 recurrent_dropout_probability=0.0, memory_cell_clip_value=None, state_projection_clip_value=None):
        ...
        with h5py.File(weight_file, 'r') as fin:
            for i_layer, lstms in enumerate(zip(self.forward_layers, self.backward_layers)):
                for j_direction, lstm in enumerate(lstms):
                    ...
                    # the bias weights
                    tf_bias = dataset['B'][...]
                    # tensorflow adds 1.0 to forget gate bias instead of modifying the parameters...
